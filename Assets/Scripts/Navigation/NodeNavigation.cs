...
public class NodeNavigation : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (navViz.isShortestPathCalculated)
        {
            navViz.drawPathForPathAtIndex(navViz.shortTestPathIndex);
        }
    }
    ...
    private void PerformPlayerToPlayerNavigation()
    {
        if (PlayerToPlayerNavigation)
        {
            ...
            {
                GameObject player1 = LocationNetworkManager.clientsRawData[0];
                GameObject player2 = LocationNetworkManager.clientsRawData[1];
